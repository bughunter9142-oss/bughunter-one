...
class ReportEngine:
    ...
    def generate(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sections = {
            ...
            "subdomain_enumeration": {"count": len(payload.get("subdomains", [])), "items": payload.get("subdomains", [])},
            "live_host_detection": {"count": len(payload.get("live_hosts", [])), "items": payload.get("live_hosts", [])},
            "public_port_and_service_identification": {"count": len(payload.get("public_ports", [])), "items": payload.get("public_ports", [])},
            "robots_txt_findings": payload.get("robots", {}),
            "sitemap_findings": payload.get("sitemap", {}),
            "javascript_collection": payload.get("javascript", {}),
            "historical_public_urls": {"count": len(payload.get("historical_urls", [])), "items": payload.get("historical_urls", [])},
            "public_directory_discovery": {"count": len(payload.get("public_directories", [])), "items": payload.get("public_directories", [])},
            "api_endpoint_discovery": {"count": len(payload.get("api_discovery", [])), "items": payload.get("api_discovery", [])},
            "authentication_surface": {"count": len(payload.get("auth_surface", [])), "items": payload.get("auth_surface", [])},
